        ...
        <div className="lg:col-span-5 space-y-6 lg:sticky lg:top-32">
          <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm space-y-5">
            ...
            <div className="space-y-4">
              ...
              <input type="text" maxLength={15} value={customer.gst} onChange={e => setCustomer({...customer, gst: e.target.value.toUpperCase()})} className="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-xs outline-none focus:ring-1 focus:ring-[#7B2D0A] uppercase" placeholder="Customer GSTIN (required for GST invoices)" />
            </div>
          </div>
            ...
            <div className="relative z-10 pt-2">
              <AnimatePresence mode="wait">
                {!isSaleSaved ? (
                  ...
                  >
                    {billingLoading ? <Loader2 className="animate-spin" size={14} /> : <><Receipt size={14} /> Record Sale & Generate Invoice <ArrowRight size={14}/></>}
                  </button>
                ) : (
